...
async def start_deeplink(message: Message, command: CommandObject):
    ...
    if user is None:
        await register_user(message, balance=4, external_user_id=str(command.args))
    text = """
...
❗Чтобы я рассказал тебе о составе блюда, на балансе бота должны быть Telegram Stars. Раз уж ты зашел, я дарю тебе 4 ⭐️! Кстати, 2 ⭐ = 1 распознавание. Начни изучать свой рацион уже сегодня! 😉 
...
async def start(message: Message):
    user = await get_user(message.from_user.id)
    if user is None:
        await register_user(message, balance=4)
    text = """
Привет! Добро пожаловать в @EasyCalloriesBot! 🤖 
Хочешь легко следить за своим питанием? 💪 Я помогу тебе!
